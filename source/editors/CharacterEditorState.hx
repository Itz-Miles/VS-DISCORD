...
class CharacterEditorState extends MusicBeatState
{
	...
	override function create()
	{
		...
		healthBarBG = new FlxSprite(30, FlxG.height - 75).loadGraphic(Paths.image('playstate/healthBar', "shared"));
		healthBarBG.scrollFactor.set();
		add(healthBarBG);
		healthBarBG.cameras = [camHUD];

		leHealthIcon = new HealthIcon(char.healthIcon, false);
		leHealthIcon.y = healthBarBG.y - leHealthIcon.height;
		add(leHealthIcon);
	...
	function reloadBGs() {
		...
		if(char.isPlayer) playerXDifference = 670;
			...
			var bg:BGSprite = new BGSprite('menuDesat', -650 + OFFSET_X - playerXDifference, 500, 1, 1);
