...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if(SONG.stage == null || SONG.stage.length < 1) {//only happens if the stage isn't there
			switch (songName)
			{
				default:
					curStage = 'stage';
		...
		switch(curStage)
		{
			//add random foreground stuff here
		}
	...
	function startSong():Void
	{
		...
		switch(curStage)
		{
			//does stuff at the start of the song
		}
	...
	function eventNoteEarlyTrigger(event:EventNote):Float {
		...
		switch(event.event) {
			// case 'some event'     Better timing so that the kill sound matches the beat intended
			//return number     Plays (number)ms before the actual position
		}
		return 0;
	}
	...
	public var skipArrowStartTween:Bool = false; // for amaomaoam
	...
	override public function update(elapsed:Float)
	{
		switch (curStage)
		{
		 // add the onupdate stuff here for stages
		}
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					if(!ClientPrefs.getGameplaySetting('practice', false) && !ClientPrefs.getGameplaySetting('botplay', false)) {
						StoryMenuChannel.weekCompleted.set(WeekData.weeksList[storyWeek], true);
						...
						FlxG.save.data.weekCompleted = StoryMenuChannel.weekCompleted;
						FlxG.save.flush();
					}
			...
			if(note.hitCausesMiss) {
				...
				if(!note.noMissAnimation)
				{
					switch(note.noteType) {
						case 'Hurt Note'| 'Instakill notes': // Deprecated, replace with 'Kill Note'(really interesting mechanics, Vs Discord team.)
