...
class PlayState extends MusicBeatState
{
	...
	override public function create() {
		...
		timeTxt.setFormat(Paths.font("minecraftia.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	function set_songSpeed(value:Float):Float {
		...
		public function reloadHealthBarColors() {
			healthBar.createFilledBar(FlxColor.fromRGB(dad.healthColorArray[0], dad.healthColorArray[1], dad.healthColorArray[2], 200), FlxColor.fromRGB(boyfriend.healthColorArray[0], boyfriend.healthColorArray[1], boyfriend.healthColorArray[2], 200));
			healthBar.updateBar();
		}
	...
	public function updateRating(note:Note = null, bounce:Bool) {
		...
		tempScore += " | Rating: " + totalRatings[0];
		tempScore += (totalRatings[0] != '?/10' ? ' (${Highscore.floorDecimal(totalRatings[1] * 100, 2)}%)' : '');
		tempScore += '\n';
	
		scoreTxt.text = tempScore;
		
		if(ClientPrefs.scoreZoom && bounce) {
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
			...
			if(value1 == 'mute') FlxG.sound.music.volume = 0;
			if(value1 == 'amplify') FlxG.sound.music.volume = 1;
	...
	function moveCameraSection(?id:Int = 0):Void {
		...
		if (gf != null && SONG.notes[curSection].gfSection) {
			camFollow.set(gf.getMidpoint().x + gf.cameraPosition[0] + girlfriendCameraOffset[0], gf.getMidpoint().y + gf.cameraPosition[1] + girlfriendCameraOffset[1]);
			tweenCamIn();
			return;
		}
	...
	public function moveCamera(isDad:Bool) {
		if(isDad) {
			camFollow.set(dad.getMidpoint().x + 150 + dad.cameraPosition[0] + opponentCameraOffset[0], dad.getMidpoint().y - 100 + dad.cameraPosition[1] + opponentCameraOffset[1]);
			tweenCamIn();
		} else {
			camFollow.set(boyfriend.getMidpoint().x - 100 - (boyfriend.cameraPosition[0] - boyfriendCameraOffset[0]), boyfriend.getMidpoint().y - 100 + boyfriend.cameraPosition[1] + boyfriendCameraOffset[1]);
	...
	private function onKeyPress(event:KeyboardEvent):Void {
		...
		if (!cpuControlled && startedCountdown && !paused && key > -1 && (FlxG.keys.checkStatus(eventKey, JUST_PRESSED) || ClientPrefs.controllerMode)) {
			...
			if(strumsBlocked[key] != true && spr != null && spr.animation.curAnim.name != 'confirm') {
				...
				switch(key) {
				case 0:
					//boyfriend.act(['move', ]); //left
					//boyfriend.heldItem.direction
				case 1:
					//boyfriend.act(['jump', ]); //down
					//boyfriend.helditem.direction
				case 2:
					//boyfriend.act(['jump', ]); //up
					//boyfriend.helditem.direction
				case 3:
					//boyfriend.act(['move', ]); //right
					//boyfriend.helditem.direction
				}
			}
		}
	...
	function playerNoteHit(note:Note):Void {
		if (!note.wasGoodHit) {
			...
			if (!singCam && !boyfriend.isDead) {
				switch (note.noteData) {
					case 0: //left
						camFollow.set(boyfriend.getMidpoint().x + boyfriend.cameraPosition[0] - 140, boyfriend.getMidpoint().y + boyfriend.cameraPosition[1] - 100);
					case 1: //up
						camFollow.set(boyfriend.getMidpoint().x + boyfriend.cameraPosition[0] - 100, boyfriend.getMidpoint().y + boyfriend.cameraPosition[1] - 60);
					case 2: //down
						camFollow.set(boyfriend.getMidpoint().x + boyfriend.cameraPosition[0] - 100, boyfriend.getMidpoint().y + boyfriend.cameraPosition[1] - 140);
					case 3: //right
						camFollow.set(boyfriend.getMidpoint().x + boyfriend.cameraPosition[0] -60, boyfriend.getMidpoint().y + boyfriend.cameraPosition[1] - 100);
				}
			}
				var daAlt = '';
	...
	override function beatHit() {
		...
		if (generatedMusic && PlayState.SONG.notes[Std.int(curStep / 16)] != null && !endingSong && !isCameraOnForcedPos && !boyfriend.isDead) {
		...
		if (gf != null && curBeat % Math.round(gf.danceMult * gf.danceEveryNumBeats) == 0 && gf.animation.curAnim != null && !gf.animation.curAnim.name.startsWith("sing") && !gf.stunned) {
				gf.dance();
			}
			if (curBeat % Math.round(boyfriend.danceMult * boyfriend.danceEveryNumBeats) == 0 && boyfriend.animation.curAnim != null && !boyfriend.animation.curAnim.name.startsWith('sing') && !boyfriend.stunned) {
				boyfriend.dance();
			}
			if (curBeat % Math.round(dad.danceMult * dad.danceEveryNumBeats) == 0 && dad.animation.curAnim != null && !dad.animation.curAnim.name.startsWith('sing') && !dad.stunned) {
				dad.dance();
			}
		/*
		switch (curStage) {
